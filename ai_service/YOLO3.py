...
def detectAndDisplay(frame, min_confidence, classes, output_layers, colors):
    ...
    # cv2.imwrite("/home/root/ObjectDetection/picture/YOLO3.jpg", img)
    cv2.imwrite("/home/root/ObjectDetection/YOLO3.jpg", img)
    return result

# 이전에 현재 경로를 저장하기
present_path = os.getcwd()
os.chdir("/home")
model_file = '/home/root/yolo3/yolov3.weights' #-- 본인 개발 환경에 맞게 변경할 것
config_file = '/home/root/yolo3/yolov3.cfg' #-- 본인 개발 환경에 맞게 변경할 것
net = cv2.dnn.readNet(model_file, config_file)
#-- 클래스(names파일) 오픈 / 본인 개발 환경에 맞게 변경할 것
classes = []
with open("/home/root/yolo3/coco.names", "r") as f:
...
for i in f:
    val = i
# 이미지 경로 설정 - 앞에서 저장한 원래 경로로 바꾸고 실행해보기
# image_path = f'{present_path}/input.png'
image_path = f"/home/root/ObjectDetection/picture/{val.strip()}"
min_confidence = 0.5 # 최소 신뢰도 임계값 설정
# 이미지 읽기
frame = cv2.imread(image_path)
my_list = detectAndDisplay(frame, min_confidence, classes, output_layers, colors)
# with open('/home/root/ObjectDetection/picture/data.txt', 'w') as file:
#     file.write('\n'.join(my_list))
